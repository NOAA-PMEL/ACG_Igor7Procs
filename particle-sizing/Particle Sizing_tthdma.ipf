...
constant general_defaultTimeBase = 120 // seconds -- for process_2d, dmps_aps, etc
constant smps_defaultTimeBase = 120 // seconds
constant dmps_defaultTimeBase = 120 // seconds
constant aps_defaultTimeBase = 120 // seconds
...
constant integration_tb_for_menu = 120
...
Menu "GraphMarquee"
	...
	submenu "Plot Average ThTDMA Distribution..."
		submenu "Number"
			"UNHEATED", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"NUMBER", "UNHEATED",0)
			"HEATED", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"NUMBER", "HEATED",0)
			"BOTH", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"NUMBER", "BOTH",0)
		end
		submenu "Surface"
			"UNHEATED", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"SURFACE", "UNHEATED",0)
			"HEATED", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"SURFACE", "HEATED",0)
			"BOTH", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"SURFACE", "BOTH",0)
		end
		submenu "Volume"
			"UNHEATED", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"VOLUME", "UNHEATED",0)
			"HEATED", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"VOLUME", "HEATED",0)
			"BOTH", ttdma_plot_avg_from_marquee("smps",integration_tb_for_menu,"VOLUME", "BOTH",0)
		end
	end
	"-"
...
Function ttdma_filter_with_valve(inst,tb,isFilter)
	string inst
	variable tb
	variable isFilter
	
	if (!acg_project_flag_is_set())
		print "You need to define the ProjectInfo before you can use this filter"
		return 0
	endif
	
	pfw_add_custom_flag("UNHEATED")
	pfw_add_custom_flag("HEATED")
	...
	wave valve_dt = root:valve:date_time
//	wave valve_dt = root:valve:StartDateTime
//	SetScale/P x valve_dt[0],1,"dat", valve 
	...
	// fix valve wave to be 0 or 5 
	valve = (valve[p] < 2) ? 0 : 5

	// 07 Sept 2017 Change the way valve filtering is done.
	variable i,j
	for (i=0; i<numpnts(dt); i+=1)
		variable index = x2pnt(valve,dt[i])
		variable init_state = valve[index] 
		string flag_val = ""
		...
		for (j=index; j<index+tb; j+=1)
			if (valve[j] != init_state)
				flag_val = "BAD"
				break
			endif
		endfor
		pfw_set_range(dt[i],dt[i]+tb,flag_val)
	endfor
	
	
//	// first pass...set all flags based on valve state
//	variable i
//	for (i=0; i<numpnts(dt); i+=1)
//		variable index = x2pnt(valve,dt[i])
//		if (valve[index] == 5) // unheated
//			pfw_set_range(dt[i],dt[i]+tb,"UNHEATED")
//		else
//			pfw_set_range(dt[i],dt[i]+tb,"HEATED")
//		endif
//	endfor		
//	
//	
//	setdatafolder root:
//	// Find valve switch times and force to nearest tb interval
//	make/D/o/n=0 valve_switches
//	make/o/n=0 valve_state
//	wave switches = valve_switches
//	wave state = valve_state
//	
//	
//	
//	variable curr_state = valve[0]
//	for (i=1;i<numpnts(valve); i+=1)
//		if (curr_state != valve[i])
//			variable cnt = numpnts(switches)
//			redimension/n=(cnt+1) switches
//			redimension/n=(cnt+1) state
//			
//			switches[cnt] = valve_dt[i]
//			state[cnt] = valve[i]
//			
//			curr_state = valve[i]
//			
//			variable diff = mod(switches[cnt],tb)
//			if (diff > tb/2)
//				switches[cnt] += tb-diff
//			else
//				switches[cnt] -= diff
//			endif
//			//print mod(switches[cnt],tb)
//			pfw_set_range(switches[cnt]-tb,switches[cnt]+tb,"BAD")
//		endif
//	endfor
	
	
	
	//pfw_set_range(start_dt,stop_dt,key)


	SetDataFolder sdf
End

//Function ttdma_plot_avg_dist_from_marquee(wn, type, withEditWin)
Function ttdma_plot_avg_from_marquee(inst,tb,moment, type,isFilter)
	string inst
	variable tb
	string moment
	string type
...
//	setdatafolder dfolder
	...
	//pfw_filter_wave(smps_dNdlogDp, "UNHEATED","smps_dNdlogDp",0, subfld="UNHEATED")
...
//	print starti, stopi		
	//starti = round(x2pnt(dw,V_left))
		...
		string wnote = ""
		wnote = replacestringbykey("INDEX_START",wn,num2str(starti))
		wnote += replacestringbykey("INDEX_STOP",wn,num2str(stopi))
		wnote += replacestringbykey("DT_START",wn,(secs2date(dt[starti],-2)+" "+secs2time(dt[starti],3)))
		wnote += replacestringbykey("DT_STOP",wn,(secs2date(dt[stopi],-2)+" "+secs2time(dt[stopi],3)))
		print wnote
		//note
...
//	endif
		...
		if (tindex == 0)
			...
			elseif (cmpstr(moment,"SURFACE")==0)
				//Label left "dW / dlogD\\Bp\\M";DelayUpdate
				Label left "dS / dlogDp";DelayUpdate
			elseif (cmpstr(moment,"VOLUME")==0)
				//Label left "dW / dlogD\\Bp\\M";DelayUpdate
				Label left "dV / dlogDp";DelayUpdate
			endif
			
			Label bottom "D\\Bp\\M (nm)"
			ModifyGraph rgb=(0,0,65280); DelayUpdate
			//Legend/C/N=text0/J/A=MC "\\s("+wn+") "+ stringfromlist(tindex,types)
			legend_text = "\\s("+gn+") "+ stringfromlist(tindex,types)
			...
			//Legend/C/N=text0/J/A=MC "\\s("+wn+") "+ stringfromlist(tindex,types)
			legend_text += "\r\\s("+gn+"#"+num2str(tindex)+") "+ stringfromlist(tindex,types)
...
Function ttdma_create_avg_dist_ccn()

	DFREF sdf = getdatafolderdfr()
	setdatafolder root:smps
	newdatafolder/o/s ccn_averages
	
	// need stringlist of sample types
	
	// ccn sample types
	wave ccn_dt = root:ccn_sample_types:ccn_datetime
	wave/T ccn_type = root:ccn_sample_types:ccn_sample_type
	wave ccn_index = root:ccn_sample_types:ccn_sample_type_index
	
//	string ccn_type_list = ""
//	variable k
//	for (k=0; k<numpnts(ccn_type); k+=1)
//		if (findlistitem(ccn_type[k],ccn_type_list)<0)
//			ccn_type_list = addlistitem(ccn_type[k],ccn_type_list,";",Inf)
//		endif
//	endfor
...
//	strconstant ccn_samp_type_list ="SEA_SWEEP_1;AMB_5_24;SEA_SWEEP_2;AMB_5_25_H;SEA_SWEEP_2_SF;AmmSulf;NaCl;STATION_2_SW;AMB_5_29_H;SEA_SWEEP_3;AMB_5_31;AMB_5_31_H;SEA_SWEEP_4;SEA_SWEEP_4_H;AMB_6_2;AMB_6_3;AMB_6_3_H;AMB_6_4_H;SEA_SWEEP_5;SEA_SWEEP_5_H;STATION_5_SW;AMB_5_20;AMB_5_21;AMB_5_25;AMB_5_29;AMB_6_4;NaCl_H;AmmSulf_H" 
	string ccn_type_list ="BAD;NORMAL;SEA_SWEEP_1;AMB_5_24;SEA_SWEEP_2;AMB_5_25_H;SEA_SWEEP_2_SF;AmmSulf;NaCl;STATION_2_SW;AMB_5_29_H;SEA_SWEEP_3;AMB_5_31;AMB_5_31_H;SEA_SWEEP_4;SEA_SWEEP_4_H;AMB_6_2;AMB_6_3;AMB_6_3_H;AMB_6_4_H;SEA_SWEEP_5;SEA_SWEEP_5_H;STATION_5_SW;AMB_5_20;AMB_5_21;AMB_5_25;AMB_5_29;AMB_6_4;NaCl_H;AmmSulf_H" 
	
	variable typ
	for (typ=1; typ<itemsinlist(ccn_type_list); typ+=1) // skip 0 which is "BAD"
	
		string type = stringfromlist(typ,ccn_type_list)
		string output_wname = "dNdlogDp_"+type
	
//		if (typ < 10)
//			output_wname += "_st0"+num2str(typ)
//		else 
//			output_wname += "_st"+num2str(typ)
//		endif
		
		ttdma_calc_avg_dist_for_type(ccn_dt, ccn_type, type, output_wname)
	
		// change this to a KEY string
		//wave out = $outname
		//note out, "TYPE="+type		
	endfor
...
Function ttdma_calc_avg_dist_for_type(types_dt, types, sel_types, output_basename)
	wave types_dt
	wave/T types
	string sel_types
	string output_basename
	
	variable types_tb = 10
	variable smps_tb = 120
	
	string smps_types = "HEATED;UNHEATED"

	wave smps_dt = root:smps:data_120sec:smps_datetime
	wave smps_dp = root:smps:data_120sec:smps_dp

	make/o/n=(0,numpnts(smps_dp)) dndlogdp_work
	wave dn_work = dndlogdp_work

	string smps_types_suffix = ""
	variable smps_t
	for (smps_t = 0; smps_t < itemsinlist(smps_types); smps_t +=1)

		if (cmpstr("HEATED",stringfromlist(smps_t,smps_types))==0)
			smps_types_suffix = "_Res"
		else
			smps_types_suffix = ""
		endif
	
		wave smps = $("root:smps:data_120sec:"+stringfromlist(smps_t,smps_types)+":smps_dNdlogDp")
		variable i
		for (i=0; i<numpnts(smps_dt); i+=1)
			variable starti = round(x2pnt(types,smps_dt[i]))
			variable stopi = round(x2pnt(types,smps_dt[i]+smps_tb))
			 
			variable add_to_avg = 1
			variable j
			for (j=starti; j<=stopi; j+=1)
				if (FindListItem(types[j],sel_types) < 0)
					add_to_avg = 0
					j = stopi+1
				endif
			endfor
			...
			if (add_to_avg)
				variable rows = dimsize(dn_work,0)
				redimension/N=(rows+1,numpnts(smps_dp)) dn_work
				dn_work[rows][] = smps[i][q]
			endif
		endfor
		
		if (dimsize(dn_work,0) > 0) 
		
			// do average
			
			make/o/n=(numpnts(smps_dp)) $(output_basename+smps_types_suffix)
			wave out = $(output_basename+smps_types_suffix)
			
			variable cols
			for (cols=0; cols<numpnts(out); cols+=1)
				make/o/n=(dimsize(dn_work,0)) tmp_col
				wave tmp = tmp_col
				tmp = dn_work[p][cols]
				wavestats/q/Z tmp
				out[cols] = V_avg
...
//	return output_basename+smps_types_suffix
	
End

Function ttdma_init(force)
	variable force

	DFREF sdf = getdatafolderdfr()

	newdatafolder/o/s root:meta
	NVAR/Z init_check = isInitialized
	if (!NVAR_Exists(init_check))
		variable/G isInitialized	= 0
	endif
	
	NVAR is_init = isInitialized
	if (force)
		is_init = 0
	endif
	...
	// try to set the Project Flag
	if (!acg_init_ProjectFlag()) // if it fails, we need to create or import ProjectInfo
		
		print "Import or create project info using:"
		print "     	acg_import_project_info()"
		print "                    - or  -"
		print "		acg_display_PIC()"
		print ""
		print "Then rerun: ttdma_init(0)"
		// acg_import_project_info() // import the projectinfo
...
Function ttdma_update()

	DFREF sdf = getdatafolderdfr()


	variable tb = smps_defaultTimeBase
	setdatafolder root:smps:$("data_"+num2str(tb)+"sec")
	pfw_filter_wave(smps_dNdlogDp, "UNHEATED","smps_dNdlogDp",0,subfld="UNHEATED")
	pfw_filter_wave(smps_dNdlogDp, "HEATED","smps_dNdlogDp",0,subfld="HEATED")
